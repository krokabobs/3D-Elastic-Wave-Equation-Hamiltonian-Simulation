...
make_fracture_geometry_trace (helper — Mesh3d hull of fracture voxels in data coords)
plot_3d_velocity_plotly
plot_3d_velocity_plotly_log
'''


def make_fracture_geometry_trace(rho_model, xmin, ymin, zmin, xmax, ymax, zmax, Nx, Ny, Nz, dx, dy, dz):
    """
    Merged Mesh3d for fracture cells (rho < rho_base).

    Cells are centered exactly like the original Scatter3d geometry:
    ``xmin + ix * dx`` (same as the cone mesh), extended by ±½ spacing along each axis
    and clipped to the physical domain so nothing sits outside ``[xmin,xmax]`` etc.
    """
    mask = rho_model < rho_base
    if not np.any(mask):
        return None

    def cell_bounds_x(ix):
        if Nx <= 1:
            return xmin, xmax
        xc = xmin + ix * dx
        return max(xmin, xc - 0.5 * dx), min(xmax, xc + 0.5 * dx)

    def cell_bounds_y(iy):
        if Ny <= 1:
            return ymin, ymax
        yc = ymin + iy * dy
        return max(ymin, yc - 0.5 * dy), min(ymax, yc + 0.5 * dy)

    def cell_bounds_z(iz):
        if Nz <= 1:
            return zmin, zmax
        zc = zmin + iz * dz
        return max(zmin, zc - 0.5 * dz), min(zmax, zc + 0.5 * dz)

    vert_index = {}
    xs, ys, zs = [], [], []

    def add_vertex(p):
        key = (round(float(p[0]), 10), round(float(p[1]), 10), round(float(p[2]), 10))
        ...
        for iy in range(ny):
            for ix in range(nx):
                if not mask[iz, iy, ix]:
                    continue
                xl, xh = cell_bounds_x(ix)
                yl, yh = cell_bounds_y(iy)
                zl, zh = cell_bounds_z(iz)
                if xl >= xh or yl >= yh or zl >= zh:
                    continue

                # +x
                if ix == nx - 1 or not mask[iz, iy, ix + 1]:
                    add_quad((xh, yl, zl), (xh, yh, zl), (xh, yh, zh), (xh, yl, zh))
                # -x
                if ix == 0 or not mask[iz, iy, ix - 1]:
                    add_quad((xl, yl, zh), (xl, yh, zh), (xl, yh, zl), (xl, yl, zl))
                # +y
                if iy == ny - 1 or not mask[iz, iy + 1, ix]:
                    add_quad((xl, yh, zl), (xh, yh, zl), (xh, yh, zh), (xl, yh, zh))
                # -y
                if iy == 0 or not mask[iz, iy - 1, ix]:
                    add_quad((xl, yl, zh), (xh, yl, zh), (xh, yl, zl), (xl, yl, zl))
                # +z
                if iz == nz - 1 or not mask[iz + 1, iy, ix]:
                    add_quad((xl, yl, zh), (xh, yl, zh), (xh, yh, zh), (xl, yh, zh))
                # -z
                if iz == 0 or not mask[iz - 1, iy, ix]:
                    add_quad((xl, yl, zl), (xl, yh, zl), (xh, yh, zl), (xh, yl, zl))
...
def plot_3d_velocity_plotly(sol, B_sqrt, rho_model, xmin, ymin, zmin, xmax, ymax, zmax, Nx, Ny, Nz, dx, dy, dz):
    ...
    fracture_trace = make_fracture_geometry_trace(
        rho_model, xmin, ymin, zmin, xmax, ymax, zmax, Nx, Ny, Nz, dx, dy, dz
    )
    ...
    fig_data = [cone_trace]
    if fracture_trace is not None:
        fig_data.append(fracture_trace)
...
def plot_3d_velocity_plotly_log(sol, B_sqrt, rho_model, xmin, ymin, zmin, xmax, ymax, zmax, Nx, Ny, Nz, dx, dy, dz, log_boost=1000):
    ...
    for t in range(n_steps):
        ...
        # Only update the Cone (trace 0); trace 1 is the static fracture mesh.
        frames.append(go.Frame(
