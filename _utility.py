...
def compute_A_elastic_3D(Nx, Ny, Nz, dx, dy, dz, bcs):
    ...
    # Flat vector order is (z, y, x): x varies fastest (C-order reshape (Nz, Ny, Nx)).
    # Kronecker order: (I_z ⊗ I_y ⊗ D_x) left-to-right so D_x acts on fastest index.
    # For v_x equation: ∂_x(σ_xx) + ∂_y(σ_xy) + ∂_z(σ_xz) → v_x
    # - σ_xx (Nx, Ny, Nz) → v_x (Nx-1, Ny, Nz): FD_x.T
    # - σ_xy (Nx-1, Ny-1, Nz) → v_x (Nx-1, Ny, Nz): FD_y
    # - σ_xz (Nx-1, Ny, Nz-1) → v_x (Nx-1, Ny, Nz): FD_z

    D_xx = sp.kron(sp.kron(sp.eye(Nz), sp.eye(Ny)), FD_x.T)  # (N_vx, N_main)

    FD_y_for_sxy = FD(dy, Ny)  # (Ny, Ny-1) - maps (Ny-1) → (Ny)
    D_xy_y = sp.kron(sp.kron(sp.eye(Nz), FD_y_for_sxy), sp.eye(Nx - 1))  # (N_vx, N_sxy)

    FD_z_for_sxz = FD(dz, Nz)  # (Nz, Nz-1) - maps (Nz-1) → (Nz)
    D_xz_z = sp.kron(sp.kron(FD_z_for_sxz, sp.eye(Ny)), sp.eye(Nx - 1))  # (N_vx, N_sxz)

    # For v_y equation: ∂_x(σ_xy) + ∂_y(σ_yy) + ∂_z(σ_yz) → v_y
    FD_x_for_sxy = FD(dx, Nx)  # (Nx, Nx-1) - maps (Nx-1) → (Nx)
    D_yx_x = sp.kron(sp.kron(sp.eye(Nz), sp.eye(Ny - 1)), FD_x_for_sxy)  # (N_vy, N_sxy)

    D_yy = sp.kron(sp.kron(sp.eye(Nz), FD_y.T), sp.eye(Nx))  # (N_vy, N_main)

    FD_z_for_syz = FD(dz, Nz)  # (Nz, Nz-1) - maps (Nz-1) → (Nz)
    D_yz_z = sp.kron(sp.kron(FD_z_for_syz, sp.eye(Ny - 1)), sp.eye(Nx))  # (N_vy, N_syz); σ_yz is (Nz-1,Ny-1,Nx)

    # For v_z equation: ∂_x(σ_xz) + ∂_y(σ_yz) + ∂_z(σ_zz) → v_z
    FD_x_for_sxz = FD(dx, Nx)  # (Nx, Nx-1) - maps (Nx-1) → (Nx)
    D_zx_x = sp.kron(sp.kron(sp.eye(Nz - 1), sp.eye(Ny)), FD_x_for_sxz)  # (N_vz, N_sxz)

    FD_y_for_syz = FD(dy, Ny)  # (Ny, Ny-1) - maps (Ny-1) → (Ny)
    D_zy_y = sp.kron(sp.kron(sp.eye(Nz - 1), FD_y_for_syz), sp.eye(Nx))  # (N_vz, N_syz)

    D_zz = sp.kron(sp.kron(FD_z.T, sp.eye(Ny)), sp.eye(Nx))  # (N_vz, N_main)
    ...
    # A = [[0, D], [-D^T, 0]] so A is anti-Hermitian (A† = -A)
...
# -- Plotting Functions --
def plot_acoustic_2D(phi, Nx, Ny, dx, dy, title='2D Wave Field', subsample=16, scale=2, width=0.003,
                     clim=(-0.15, 0.15)):
